...
public class UdpClientTwoClients : MonoBehaviour {
    ...
    // Referência para a câmera que seguirá o jogador local
    public Camera gameCamera;
    
    // Configurações da câmera
    [Header("Camera Settings")]
    public float cameraLag = 0.1f;
    public Vector3 cameraOffset = new Vector3(0, 0, -10); // Offset padrão para câmera 2D

    void Start() {
        client = new UdpClient();
        serverEP = new IPEndPoint(IPAddress.Parse("10.57.1.150"), 5001);
        ...
        // Se não foi atribuída uma câmera, usa a principal
        if (gameCamera == null) {
    ...
    void Update() {
        ...
        // Atualiza posição da câmera para seguir o jogador local
        UpdateCameraPosition();
    }
    
    void UpdateCameraPosition() {
        if (gameCamera != null && localCube != null) {
            Vector3 targetPosition = localCube.transform.position + cameraOffset;
